...
in_confirm = read_csvfile('/Users/averyfield/Downloads/avery_csv.csv')
...
for i in range(1, len(final)):
    my_dict[str(i)] = final[i]

#combine chunks with similar topics
new_chunks = []
#for j in range(0, len(n_topic)):
for item in my_dict.items():
    i = item[1]
    for j in range(len(i)):
        if j < (len(i) - 1):
            if len(set(i[j]).intersection(set(i[j + 1]))) > 2:
                new_chunks.append(list(set(i[j] & i[j + 1])))
            else:
                new_chunks.append(i[j]) 
        else:
            new_chunks.append(i[j])  
